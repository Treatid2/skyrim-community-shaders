...
#include "fsr_eye_dispatch_types_under_test.h"
	...
	{
		(void)runtimeContextsCompatible;
...
namespace
{
	...
	void HostFallbackPreservesContextBounds()
	{
		...
		for (bool runtimeSelected : { false, true }) {
			for (uint32_t blocker = 0; blocker < 7; ++blocker) {
				auto& upscaling = Reset();
				auto& provider = upscaling.fidelityFX;
				EnableHost(provider);
				provider.plan.selected = runtimeSelected;
				provider.plan.providerSetupDeferred = false;
				provider.simulatedRuntimeResult = Lifecycle::Failed;
				auto params = Region(0, 504);
				if (blocker == 0)
					provider.fsrContextMaxRenderWidth = params.inputWidth - 1;
				if (blocker == 1)
				...
				if (blocker == 6)
					provider.fsrContextCount = 1;
				Require(
					upscaling.DispatchVendorEyeRegion(UpscaleMethod::kFSR, params) == Result::Failed &&
						provider.hostCalls == 0 &&
						provider.runtimeCalls == (runtimeSelected ? 1u : 0u),
					"An incompatible host reached dispatch outside the Pending path");
			}
		}
		...
		for (uint32_t eye = 0; eye < stereo.size(); ++eye) {
			const auto params = Region(eye, 504);
			stereo[eye] = {
				.contextIndex = eye,
		...
		for (bool runtimeSelected : { false, true }) {
			...
			provider.plan.providerSetupDeferred = false;
			provider.simulatedRuntimeResult = Lifecycle::Failed;
			provider.fsrContextMaxRenderWidth = stereo[0].renderWidth - 1;
			Require(provider.UpscaleStereoRegions(stereo) == FidelityFX::StereoUpscaleResult::Failed &&
						provider.hostCalls == 0 &&
						provider.runtimeCalls == (runtimeSelected ? 1u : 0u),
				"Stereo dispatch exposed an incompatible host as NotHandled");
		}
